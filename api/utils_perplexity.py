...
【文体のルール】
- 難しい占術用語・英語略語は使わない（例：「トランジット」「ハウス」「ASC」「MC」「IC」などは禁止）
...
【出力JSONスキーマ】
{{
"destiny_map": "生まれ持った流れとこれからのテーマを250文字程度で。「あなたが生まれた日、星たちは…」から始め、ユーザーの悩みとつながる形で運命の流れを語る。専門用語なし。",
"past": "これまでどんな資質や経験を積んできたかを150文字程度で。「幼い頃から…」や「これまで…してきたあなたは」のように語りかける。ユーザーの悩みに関連する強みを含める。",
"present_future": "今の状況とこれからの変化を200文字程度で。「いまのあなたは…という状態にあります」と今を認め、「これから…することで」と具体的な行動につながるヒントを伝える。ユーザーの悩みの場面に直接沿って書く。",
"element_diagnosis": "エネルギーのバランスを日常の言葉で150文字程度で。「最近、〇〇な気持ちになることはありませんか？」のように問いかけてから、今の状態を説明する。ユーザーの悩みに絡めた具体例を1つ入れる。",
"oracle_message": "引いたカード「{oracle_result['card']['name']}」の{position_str}のメッセージを150文字程度で。「このカードはあなたに…と伝えています」と語りかけ、ユーザーの悩みへの気づきにつなげる。",
"bracelet_proposal": "ブレスレットを身につけることで日常がどう変わるか150文字程度で。ユーザーの悩みに関連する具体的な場面（例：「そのことで悩んでいたとき…」）を描く。",
"stone_support_message": "石がどのようにサポートするか200文字程度で。「この石は…なあなたの気持ちに寄り添います」と擬人化して語りかける。ユーザーの悩みの核心に正面から触れること。",
"daily_advice": "今日からできる具体的なアクションを3つ、それぞれ25文字以内で。カンマ区切り。ユーザーの悩みに関連する行動を含める。",
"lucky_color": "今日のラッキーカラー（1色、日本語）",
"affirmation": "あなたへの肯定の言葉を50文字程度で。「私は…」で始める。ユーザーの悩みが少し楽になるような言葉にする。",
"chosen_stone": {{
  "name": "選ばれた石の名前（日本語）",
  "reason": "ユーザーの悩み・ホロスコープ・カードの観点からその石が必要な理由を1－2文で。悩みへの言及を必ず含める。"
}},
...
def generate_bracelet_reading(user_input: dict, chart_data: dict = None) -> dict:
    ...
    system_msg = (
        ...
        "【最重要ルール】\n"
        "ユーザーが自分の言葉で書いた「具体的な悩み」が鑑定の中心です。\n"
        "ホロスコープや星座はその悩みを読み解くための補足情報として使い、\n"
        "すべての鑑定文でユーザーの悩みに直接応えてください。\n"
        "ユーザーの言葉を拾い、「〜とお感じなのですね」「〜な状況の中で」のように\n"
        "その人固有の状況に寄り添った言葉で語りかけてください。\n\n"
        "【前提条件】\n"
        "3. ユーザーの「悩み詳細」を最優先で読み取り、共感のこもった鑑定を行うこと。\n"
