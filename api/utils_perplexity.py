...
5. 各セクションは詳しく、読みごたえのある内容にしてください。
6. 【】のような見出しマークは一切使わず、自然な文章だけで書いてください。重要な箇所は**で囲って強調してください。

【鑑定文の構成】
各セクションは以下のとおりで、段落を分けて出力してください：
- destiny_map: 全体のテーマ・運命の地図（400文字程度）。見出し【】は不要、本文のみ。
- past: 生まれ持った資質・これまでの流れ（300文字程度）。見出し【】は不要、本文のみ。
- present: 今の課題・テーマ（300文字程度）。見出し【】は不要、本文のみ。
- future: これから開いていく可能性（300文字程度）。見出し【】は不要、本文のみ。
- element_diagnosis: 火・地・風・水のバランスと不足要素（300文字程度）。見出し【】は不要、本文のみ。
- oracle_message: カードの正逆を踏まえたメッセージ（300文字程度）。見出し【】は不要、本文のみ。
- bracelet_proposal: どんな意図で石を選んだか、どんな願いをサポートするか（400文字程度）。見出し【】は不要、本文のみ。
- stone_support_message: 「あなたはこういう状況であなたにはこういう石の魔法のサポートが必要です」というメッセージ（200文字程度）。ユーザーの具体的な状況と、その状況に対して石がどのようにサポートするのかを説明してください。
"""
...
{{
  "destiny_map": "全体のテーマ・運命の地図を400文字程度で。【】見出しは絶対に使わず、自然な文章だけで書いてください。重要な言葉は**で囲んで強調。",
  "past": "生まれ持った資質・これまでの流れを300文字程度で。【】見出しは絶対に使わず、自然な文章だけで書いてください。重要な言葉は**で囲んで強調。",
  "present": "今の課題・テーマを300文字程度で。【】見出しは絶対に使わず、自然な文章だけで書いてください。重要な言葉は**で囲んで強調。",
  "future": "これから開いていく可能性を300文字程度で。【】見出しは絶対に使わず、自然な文章だけで書いてください。重要な言葉は**で囲んで強調。",
  "element_diagnosis": "火・地・風・水のバランスと不足要素、アドバイスを300文字程度で。【】見出しは絶対に使わず、自然な文章だけで書いてください。重要な言葉は**で囲んで強調。",
  "oracle_message": "引いたカード「{oracle_result['card']['name']}」の{position_str}の詳細メッセージを300文字程度で。【】見出しは絶対に使わず、自然な文章だけで書いてください。重要な言葉は**で囲んで強調。",
  "bracelet_proposal": "どんな意図で石を選び、どんな願いをサポートするか、400文字程度で。【】見出しは絶対に使わず、自然な文章だけで書いてください。重要な言葉は**で囲んで強調。",
